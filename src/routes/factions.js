...
router.post('/', async (req, res, next) => {
  try {
    ...
    const record = await db.create(COLLECTION, { name, alignment, description, goals, headquarters, leader, notes });
...
router.put('/:id', async (req, res, next) => {
  try {
    ...
    const { name, alignment, description, goals, headquarters, leader, notes } = req.body;
    const record = await db.update(COLLECTION, req.params.id, { name, alignment, description, goals, headquarters, leader, notes });
    res.json(record);
  } catch (error) {
    next(error);
  }
