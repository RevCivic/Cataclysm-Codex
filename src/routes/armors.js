...
router.post('/', async (req, res, next) => {
  try {
    ...
    const record = await db.create(COLLECTION, { name, type, level, eac_bonus, kac_bonus, max_dex, armor_check_penalty, speed_adjustment, upgrade_slots, bulk, price, description });
...
router.put('/:id', async (req, res, next) => {
  try {
    ...
    const { name, type, level, eac_bonus, kac_bonus, max_dex, armor_check_penalty, speed_adjustment, upgrade_slots, bulk, price, description } = req.body;
    const record = await db.update(COLLECTION, req.params.id, { name, type, level, eac_bonus, kac_bonus, max_dex, armor_check_penalty, speed_adjustment, upgrade_slots, bulk, price, description });
    res.json(record);
  } catch (error) {
    next(error);
  }
