...
router.post('/', async (req, res, next) => {
  try {
    ...
    const record = await db.create(COLLECTION, { name, model, size, speed, maneuverability, shields, hull_points, crew, weapons: shipWeapons, affiliation, description, notes });
...
router.put('/:id', async (req, res, next) => {
  try {
    ...
    const { name, model, size, speed, maneuverability, shields, hull_points, crew, weapons: shipWeapons, affiliation, description, notes } = req.body;
    const record = await db.update(COLLECTION, req.params.id, { name, model, size, speed, maneuverability, shields, hull_points, crew, weapons: shipWeapons, affiliation, description, notes });
    res.json(record);
  } catch (error) {
    next(error);
  }
