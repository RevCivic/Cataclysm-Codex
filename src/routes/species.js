...
router.post('/', (req, res) => {
  const { name, home_world, description, traits, size, type, notes } = req.body;
  if (!name) return res.status(400).json({ error: 'name is required' });
  const record = db.create(COLLECTION, { name, home_world, description, traits, size, type, notes });
...
router.put('/:id', (req, res) => {
  ...
  const { name, home_world, description, traits, size, type, notes } = req.body;
  const record = db.update(COLLECTION, req.params.id, { name, home_world, description, traits, size, type, notes });
  res.json(record);
});
